...
# Join eukulele, eggnog, and salmon outputs together. We can "join" the dataframes by the contigID.
tax.ko <- full_join(tax_fin2,eggnog_fin)
tax.ko.count <- left_join(tax.ko, salmon_fin2)
tax.ko.count <- subset(tax.ko.count, NumReads !=0)

# Load in cluster number (i.e., MMSeqs data)
clus <- read.delim("clusters.tsv", header=FALSE)
colnames(clus) <- c("contigID","Cluster")
clus$Cluster <- as.factor(clus$Cluster)

# Join cluster information with taxonomy, function, and salmon counts
all <- left_join(tax.ko.count,clus)
...
wide.data <- dcast(all, contigID+Taxonomy+KO+Cluster~SampleName,fun.aggregate = sum,value.var= "NumReads")
